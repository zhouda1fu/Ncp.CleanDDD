...
public class RefreshTokenEndpoint : Endpoint<RefreshTokenRequest, ResponseData<RefreshTokenResponse>>
{
    ...
    {
        // 1. 查询：验证用户凭据
        var userId = await _userQuery.GetUserIdByRefreshTokenAsync(req.RefreshToken, ct) ?? throw new KnownException("无效的token");
...
public class RefreshTokenSummary : Summary<RefreshTokenEndpoint>
{
    ...
    {
        Response<RefreshTokenResponse>(200, "刷新token");
    }
}